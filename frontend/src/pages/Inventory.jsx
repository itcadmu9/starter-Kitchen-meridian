...
import { createReorderRequest } from '../services/reorderService'
...
export default function Inventory() {
  ...
  useEffect(loadItems, [outlet, page])


  const statusFor = (item) => Number(item.quantity) <= 0 ? ['Out of stock', 'status-out'] : Number(item.quantity) <= Number(item.reorder_threshold) ? ['Low stock', 'status-low'] : ['In stock', 'status-good']
  ...
  const openReorderForm = (item) => {
    setReorderItem(item)
    setReorderQuantity(String(Number(item.reorder_threshold) * 2 || 1))
  }

  const submitReorder = async (event) => {
    event.preventDefault()
    try {
      await createReorderRequest({ property_id: reorderItem.property_id, inventory_item_id: reorderItem.id, quantity: Number(reorderQuantity) })
      setReorderItem(null)
      setReorderQuantity('')
    } catch (err) {
      setError(err.message)
  ...
  return (
    <div className="page">
      ...
      <section className="panel table-panel"><div className="panel-heading"><div><h2>All ingredients</h2><span className="muted">{total} items</span></div><div className="panel-heading-actions"><button className="outline-button" type="button" onClick={loadItems}>Refresh</button><button className="primary-button" type="button" onClick={openAddForm}>Add ingredient</button></div></div><div className="table-scroll"><table><thead><tr><th>Ingredient</th><th>Outlet</th><th>Quantity</th><th>Reorder threshold</th><th>Status</th><th>Actions</th></tr></thead><tbody>{items.map((item) => { const [status, statusClass] = statusFor(item); return <tr key={item.id}><td className="strong-cell">{item.name}</td><td>{item.outlet}</td><td>{Number(item.quantity).toFixed(2)} {item.unit}</td><td>{Number(item.reorder_threshold).toFixed(2)} {item.unit}</td><td><span className={`status-badge ${statusClass}`}>{status}</span></td><td>{user?.role === 'MANAGER' ? <button className="outline-button" type="button" onClick={() => openReorderForm(item)}>Reorder</button> : <button className="outline-button" type="button" onClick={() => setStockItem(item)}>Add stock</button>}</td></tr> })}</tbody></table></div><div className="pagination"><span>Showing {total ? page : 0} of {Math.max(1, Math.ceil(total / PAGE_SIZE))}</span><div><button className="page-button" disabled={page === 1} onClick={() => setPage(page - 1)}>Previous</button><button className="page-button" disabled={page * PAGE_SIZE >= total} onClick={() => setPage(page + 1)}>Next</button></div></div></section>
      ...
      {reorderItem && <div className="modal-backdrop"><form className="modal" onSubmit={submitReorder}><p className="eyebrow">Purchasing</p><h2>Reorder {reorderItem.name}</h2><p>{reorderItem.name} at {reorderItem.outlet} currently has {Number(reorderItem.quantity).toFixed(2)} {reorderItem.unit} (reorder threshold {Number(reorderItem.reorder_threshold).toFixed(2)} {reorderItem.unit}).</p><label>Quantity to order<input autoFocus min="0.01" step="0.01" type="number" value={reorderQuantity} onChange={(event) => setReorderQuantity(event.target.value)} required /></label><div className="modal-actions"><button className="outline-button" type="button" onClick={() => setReorderItem(null)}>Cancel</button><button className="primary-button" type="submit">Submit reorder</button></div></form></div>}
